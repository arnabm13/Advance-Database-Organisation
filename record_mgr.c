...
/* Record Manager: Below functions are defined to create a table , insert records, get records, update records.
The client creates a table, does operation of insert update and delete record.  The program also uses buffer manager for better memory managment. It also gives the result of a query fired to the table.
*/
...
//structure to contains the information of a table
...
// Function to convert a table to string..
...
// function to get table information from a string containg the structure of the table
...
// function to get schema size of a table

int getSchemaSize (Schema *schema){
    int i, size = 0;

    size = sizeof(int)                        
    + sizeof(int)*(schema->numAttr)     
    + sizeof(int)*(schema->numAttr)   
    + sizeof(int)                      
...
// Function to deserialize record from a serialized record stored in a page.
...
/*****************************Functions to create, open,close, insert, update and delete a table **********************/

extern RC createTable (char *name, Schema *schema){

 int status;
 SM_FileHandle fh;
//First we check if a file already exists with the file name.
    if( access(name, F_OK) != -1 ) {
        return RC_TABLE_ALREADY_EXITS;
    }
    
    /* The if there is no file, we the table name, then we create a file with table name, the file contains the numper of pages in the page0.
page-1 contains the information of the table. Like slot size, filesize, number of the page from where the record starts. etc.

*/
    if ((status=createPageFile(name)) != RC_OK)
        return status;

    int schema_size = getSchemaSize(schema); //number of bytes the schema will take
    ...
    info->recordStartPage = file_size + 2;
    info->slotSize = slot_size;
    info->recordLastPage = file_size + 2;
    ...
    // In the page 2, we put the schema of the table. The schema of the table contains the information of structure of the table.
//Number and name of attributes, data type and length
...
// function to open a table it first checks if the table to open exists or not?
...
// function to close table already opened
...
// function get record from a table
...
// function to scan through a table
